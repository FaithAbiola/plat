...
const fetchPayroll = async () => {
  loading.value = true;
  const response = await request(payrollStore.getSingle(id.value as string),loading);
  ...
  if (successResponse && typeof successResponse !== "undefined") {
    ...
    responseData.value.employees.forEach((element:any, index:number) => {
      const employee_payment = responseData.value.payments.find((data:any) => data.employee_id == element.id) ?? null;
      if(employee_payment){
        return responseData.value.employees[index] = {...element, employee_payment}
      }else{
        return responseData.value.employees[index] = {...element, employee_payment}
      }
    });
    ...
    // console.log('---')
    failedPayment.value = responseData.value.payments.find((data:any) => data.status === 'failed') ? true : false;
        ...
        <div class="flex space-x-5">
          ...
          <div>
            <h3 class="font-bold text-2xl whitespace-nowrap">
              {{ responseData ? responseData.month : "..." }} Salary Payment
            </h3>
      ...
      <div class="grid lg:grid-cols-3 gap-3">
        ...
        <div class="h-full rounded-lg p-4 bg-blue-lighter space-y-8">
          ...
          <div class="space-y-5">
            <div class="items-center">
              <div class="flex flex-col">
                <span class="text-sm">₦ {{ currency(responseData?.salaries) }} </span>
            ...
            <div class="align-middle inline-block min-w-full">
              <div class="overflow-hidden sm:rounded-lg">
                ...
                <table class="min-w-full table-fixed">
                  <thead  v-if="responseData?.payments && responseData?.payments[0]"
                  ...
                  >
                    <tr>
                      ...
                      <th scope=" col" class="py-4 text-left">Base Salar</th>
                  ...
                  <tbody
                    ...
                    v-if="responseData?.employees && responseData?.employees[0]"
                  >
                    <tr
                      v-for="(employee, index) in responseData?.employees"
                      :key="index"
                    ...
                    >
                      <td class="py-4 whitespace-nowrap">
                        <div class="flex items-center space-x-3 flex-shrink-0">
                          <FCheckBoxComp
                            :value="checkState(employee.employee_payment?.id)"
                            @click="handlePayment(employee.employee_payment?.id)"
                            :disabled="employee.employee_payment?.status !== 'failed'"
                          />
                          <div class="flex flex-col">
                      ...
                      <td class="py-4 whitespace-nowrap">
                        <div class="font-normal text-left flex flex-col">
                          <span class="text-sm font-semimedium"
                            >₦ {{
                              currency(employee.salary.total)
                            }}</span
                          >
                        </div>
                      </td>
                      <td class="py-4 flex whitespace-nowrap">
                        <div class="font-normal text-left flex flex-col">
                          <span class="text-sm font-semimedium"
                            >₦ {{ employee.payments?.bonus ? currency(employee.payments?.bonus) : 0 }}</span
                          >
                          <!-- <span class="text-xs text-gray-rgba-3"
                      ...
                      <td class="py-4 items-center text-left whitespace-nowrap">
                        <div class="font-normal flex flex-col">
                          <span class="text-sm font-semimedium"
                            >₦ {{
                              employee.payments?.deductions ? currency(employee.payments?.deductions) : 0
                            }}</span
                          >
                        </div>
                      </td>
                      <td class="py-4 items-center text-left whitespace-nowrap">
                        <div class="font-normal flex flex-col">
                          <span class="text-sm font-semimedium"
                            >₦ {{ employee.payments?.tax ? currency(employee.payments.tax) : 0 }}</span
                          >
                        </div>
                      </td>
                      <td class="py-4 items-center text-left whitespace-nowrap">
                        <div class="font-normal flex flex-col">
                          <span class="text-sm font-semimedium"
                            >₦{{ employee.salary.total ? currency(employee.salary.total) : 0 }}</span
                          >
                        </div>
                      </td>
                      <td class="py-4 items-center text-left whitespace-nowrap">
